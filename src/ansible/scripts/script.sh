...
emerge-world() {
    ...
    # Add needed packages.
    PKG="sys-kernel/gentoo-sources app-editors/vim dev-util/cdiff sys-kernel/dracut sys-fs/btrfs-progs app-portage/gentoolkit app-portage/eix app-portage/genlop sys-apps/gptfdisk app-misc/screen net-dns/bind-tools app-portage/cpuid2cpuflags"
    if [ -n "$BOOT_DEV" ]; then
...
make-kernel() {
    ...
    if [ -n "$BOOT_MBR_DEV" ]; then
        ...
        efibootmgr -v -c -d "$BOOT_DISK_DEV" -p "$BOOT_PART_NUM" -L "linux-$V" -l "\\vmlinuz-$V" -u "initrd=\\initramfs-$V.img ro root=UUID=$ROOT_FS_UUID rootflags=subvol=root,noatime rd.luks.uuid=$ROOT_DEV_UUID systemd.unified_cgroup_hierarchy=0"
    fi
}
